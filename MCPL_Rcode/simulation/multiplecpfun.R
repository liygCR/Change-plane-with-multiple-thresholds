...
##
smooth <- function(ZZ, s.est, ini.a = c(qnorm(0.3), seq(qnorm(0.6), 1, length.out = s.est-1)), 
...
#     split.fit <- NULL
#     for(k in 1:length(c)) {
#       split.fit[[k]] <- split.fun(X, Y, c[k])
#     }
#     BIC <- sapply(split.fit, function(x){x$BIC}, simplify = T)
#     mcp.cp1 <- sapply(split.fit, function(x){x$mcp.cp1}, simplify = F)
#     mcp.cp1 <- mcp.cp1[[which.min(BIC)]]
#     s.est <- length(mcp.cp1)
#     sm.obj <- smooth(ZZ = ZZo, s = s.est, ini.theta = theta.hat)
#     
#     
#   } else {
#     

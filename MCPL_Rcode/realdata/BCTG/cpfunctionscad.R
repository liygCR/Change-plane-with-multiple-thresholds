...
lmres<-function(Y,X,delta)
{
  ...
  if(length(id)>=dim(X)[2])
  {
    ...
    X3=as.matrix(X3)
    lmr=lm(Y3~W3+X3-1)
    return(list(length(Y)*sum((lmr$residuals)^2),lmr$coefficients,sqrt(length(Y))*Y3,sqrt(length(Y))*X3,sqrt(length(Y))*W3))
  }
  if(length(id)<dim(X)[2])
    return(NULL)
}
...
cumcp<-function(Y,X,delta)
{
  ...
  if(n1>2*p)
  {
    for(i in delta1[p+1]:delta1[n1-p])
      cp[i]=lmres(Y[1:i],X[1:i,],delta[1:i])[[1]]+lmres(Y[(i+1):n],X[(i+1):n,],delta[(i+1):n])[[1]]
...
TSMCP<-function(Y,X,delta,c)
{
  ...
  for(i in 2:(q-1))
  {
    tt=lmres(Y[id1[n1-(q-i+1)*m+1]:id1[n1-(q-i)*m]],X[id1[n1-(q-i+1)*m+1]:id1[n1-(q-i)*m],],delta[id1[n1-(q-i+1)*m+1]:id1[n1-(q-i)*m]])
  ...
  i=q
  tt=lmres(Y[id1[n1-(q-i+1)*m+1]:n],X[id1[n1-(q-i+1)*m+1]:n,],delta[id1[n1-(q-i+1)*m+1]:n])
  ...
  if(q>10)
  {
    ...
    if(d1>=1)
    {
      ...
      {
        t=mcp.cp1[i]
        id=c(id1[n1-(q-t+2)*m+1]:id1[n1-(q-t-1)*m])
        cp.t=cumcp(Y[id],X[id,],delta[id])[[1]]
        if(cp.t>0)
          mcpcss.cp=c(mcpcss.cp,cp.t+id1[n1-(q-t+2)*m+1]-1)
      }
  ...
  if(length(mcpcss.cp)>=1&&mcpcss.cp[1]!=0)
  {
    ...
    for(i in 2:length(id2))
    {
      tt=lmres(Y[(id2[i-1]+1):id2[i]],X[(id2[i-1]+1):id2[i],],delta[(id2[i-1]+1):id2[i]])
    ...
    mcp.coef.v.m=matrix(c(mcp.coef),length(id2),p+1,byrow=TRUE)
    ...
    if(length(mcp.m.id)>0)
    {
      ...
      id.tmp <- c(mcp.m.id-1)
      if(id.tmp[1] ==0){id.tmp[1] = id.tmp[1]+1}
      mcpcss.cp=mcpcss.cp[-id.tmp]
    }
  }
  ...
  return(list(cp=mcpcss.cp,coef=mcp.coef,sigma=sigmaep,residuals=ep,Yn=Yn,Xn=Xn))
}